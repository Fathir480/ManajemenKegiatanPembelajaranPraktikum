...
import '../../admin/absensi/absensi.css'; // Re-use the matrix styles
...
export default function AsistenNilai() {
  ...
  // Fixed layout columns (P1-P6 and A1-A3 are editable, UTS and UAS are readOnly)
  const fixedColumns = [
    { key: 'p1', label: 'P 1', type: 'praktikum', index: 1, readOnly: false },
    { key: 'p2', label: 'P 2', type: 'praktikum', index: 2, readOnly: false },
    { key: 'p3', label: 'P 3', type: 'praktikum', index: 3, readOnly: false },
    { key: 'p4', label: 'P 4', type: 'praktikum', index: 4, readOnly: false },
    { key: 'p5', label: 'P 5', type: 'praktikum', index: 5, readOnly: false },
    { key: 'p6', label: 'P 6', type: 'praktikum', index: 6, readOnly: false },
    { key: 'a1', label: 'A 1', type: 'asistensi', index: 1, readOnly: false },
    { key: 'a2', label: 'A 2', type: 'asistensi', index: 2, readOnly: false },
    { key: 'a3', label: 'A 3', type: 'asistensi', index: 3, readOnly: false },
    { key: 'uts', label: 'UTS', type: 'uts', readOnly: true },
    { key: 'uas', label: 'UAS', type: 'uas', readOnly: true },
  ...
  const fetchNilaiData = async (kelasId) => {
    try {
      ...
      setSuccess('');
      setUnsavedChanges({});
      const data = await api.get(`/asisten/nilai/kelas/${kelasId}`);
  ...
  const saveChanges = async () => {
    ...
    try {
      setSaving(true);
      setError('');
      await api.post('/asisten/nilai/bulk', { updates });
  ...
  return (
    <DashboardLayout title="Student Grades (Assistant)">
      <div className="page-header">
        <div className="page-header-left">
          <h1 style={{ fontSize: '28px' }}>Manage Grades</h1>
          <p className="page-subtitle">View class grade recap and input Practicum/Assistance scores directly</p>
        </div>
      ...
      <div className="card mb-6">
        <div className="absensi-controls" style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px', flexWrap: 'wrap' }}>
          <div className="form-group mb-0" style={{ minWidth: '320px', flexGrow: 0 }}>
            <label className="form-label text-mono" style={{ fontSize: '11px', textTransform: 'uppercase', marginBottom: '6px' }}>Select Assisted Class</label>
            {loadingClasses ? (
              <div className="text-muted text-mono" style={{ fontSize: '12px' }}>Loading your classes...</div>
            ) : classes.length === 0 ? (
              <div className="text-muted text-mono" style={{ fontSize: '12px', color: 'var(--error)' }}>You are not assigned to assist any classes yet.</div>
            ) : (
              <select 
                ...
                {students.map(std => (
                  <tr key={std.id}>
                    ...
                    {fixedColumns.map(col => {
                      ...
                      if (col.readOnly) {
                        return (
                          <td key={col.key} style={{ textAlign: 'center', verticalAlign: 'middle', padding: '10px 4px', color: 'var(--ink)', backgroundColor: 'var(--surface-soft)' }}>
